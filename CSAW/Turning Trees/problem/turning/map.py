...
class Map(object):
    ...
    def load(file, px, py):
        ...
        height = m.shape[0]
        print(width, height)
        print(m[height-3][width-3])
        wy, wx = np.where(m == 3)

        em = EntityManager(width, height)
        for y, x in zip(wy, wx):
            em.add(Log(x, y))
        # for i in range(len(em.buckets)):
        #     for j in range(len(em.buckets[i])):
        #         if j == 0:
        #             print(i, j, len(em.buckets[i][j]))
    ...
    def render_player(self, screen, camera):
        nx, ny, nw = self._coords_to_screen(
            screen, self.player.x, self.player.y, camera)
        pygame.draw.rect(
            screen,
            PLAYER_COLOR,
            (
                ...
                if not e.is_raft:
                    if e.orientation is LogOrientation.STANDING:
                        ...
                        pygame.draw.rect(
                            screen,
                            TREE_COLOR,
                            (nx+(nw*0.1), ny+(nw*TREE_SIZE), int((nw*0.8)+1),
                             int((nw*(1 - (2*TREE_SIZE)))+1))
                        )
                    elif e.orientation is LogOrientation.VERTICAL:
                        pygame.draw.rect(
                            screen,
                            TREE_COLOR,
                            (nx+(nw*TREE_SIZE), ny+(nw*0.1),
                             int((nw*(1-(2*TREE_SIZE)))+1), int((nw*0.8)+1))
                        )
                else:
                    # render raft
                    pygame.draw.rect(
                        screen,
                        TREE_COLOR,
                        (nx+(nw*0.1), ny+(nw*0.15),
                         int((nw*0.8)+1), int((nw*0.3)+1))
                    )
                    pygame.draw.rect(
                        screen,
                        TREE_COLOR,
                        (nx+(nw*0.1), ny+(nw*0.55),
                         int((nw*0.8)+1), int((nw*0.3)+1))
                    )
    ...
    def raw_push_log(self, log, tx, ty, ent):
        if ent is None:
            if self.dat[ty][tx] == Tiles.WATER:
                self.transition = Transition(
                    [TreeIntoWaterAnimation(log, tx, ty)])
            ...
            if self.dat[ty][tx] == Tiles.WATER:
                self.transition = Transition(
                    [MakeRaftAnimation(log, ent, tx, ty)])
            elif self.dat[ty][tx] == Tiles.LAND:
                pass
            elif self.dat[ty][tx] == Tiles.STUMP:
                pass
